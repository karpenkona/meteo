...
function makeRss(){
    ...
    daoNews.getActiveN(1000).then(function (data) {
        ...
        data.forEach(function (i, elem)
        {
            ...
            var item = xml.ele('url')
                .ele('loc', 'https://meteohub.pro/news/'+i._id).up()
                .ele('lastmod', new Date(i.pubDate).toISOString()).up()
        ...
        //xml.end({ pretty: true })
        fs.writeFile('../static/rss/sitemap.xml', xml.end({ pretty: true }), function (err) {
            if (err) throw err;
